...
;(function() {
    ...
    ctx.prototype.__applyStyleToCurrentElement = function(type) {
        ...
        if (type === "stroke") {
            if (this.__lineDash && this.__lineDash.length > 0) {
                this.__currentElement.setAttribute("stroke-dasharray", this.__lineDash.join(","));
            } else {
                this.__currentElement.removeAttribute("stroke-dasharray");
            }
        }
    };
    ...
    ctx.prototype.__parseFont = function() {
        ...
        if (fontPart) {
            ...
            var sizeMatch = this.font.match(/([\d\.]+(?:px|pt|em|%))/);
            var size = sizeMatch ? sizeMatch[1] : '10px';
            var family = this.font.replace(/.*?[\d\.]+(?:px|pt|em|%)\s*/, '') || 'sans-serif';
            var weight = this.font.indexOf('bold') !== -1 ? 'bold' : 'normal';
            var style = this.font.indexOf('italic') !== -1 ? 'italic' : 'normal';
            data = {
                style : style,
                size : size,
                family : family,
    ...
    ctx.prototype.setLineDash = function(segments) {
        this.__lineDash = segments;
    };
